...
public class DList extends List {
  ...
  public void insertFront(Object item) {
    // Your solution here.  Similar to Homework 4, but now you need to specify
    //   the `list' field (second parameter) as well.
    DListNode insertFrontNode = newNode(item, this, null, null);
    if (size==0) {
      insertFrontNode.prev = head;
      insertFrontNode.next = head;
      head.prev = insertFrontNode;
      head.next = insertFrontNode;
    } else {
      head.next.prev = insertFrontNode;
      insertFrontNode.next = head.next;
      head.next = insertFrontNode;
      insertFrontNode.prev = head;
    }
    size++;
