...
class Date {
  ...
  public boolean isAfter(Date d) {
      if (!(this.isBefore(d)) && !(this.year == d.year &&
  ...
  public int difference(Date d) {
    ...
    int yearGap;
    if (this.isAfter(d)) {
        yearGap = this.year - d.year;
        if (yearGap == 0){
            diffDay = (this.dayInYear() - d.dayInYear());
        } else {
            if (isLeapYear(d.year)){
                diffDay += (366 - d.dayInYear());
            } else {
                diffDay += (365 - d.dayInYear());    
            }
            for (int i = 1; i <= yearGap; i++) {
                if ((d.year+i) == this.year) {
                    diffDay += this.dayInYear();
                } else {
    ...
    if (this.isBefore(d)) {
        yearGap = d.year - this.year;
        if (yearGap == 0) {
            diffDay = (d.dayInYear() - this.dayInYear());
        } else {
            if (isLeapYear(this.year)) {
                diffDay += (366 - this.dayInYear());
            } else {
                diffDay += (365 - this.dayInYear()); 
            }
            for (int i = 1; i <= yearGap; i++) {
                if ((this.year+i) == d.year) {
                    diffDay += d.dayInYear();
                } else {
