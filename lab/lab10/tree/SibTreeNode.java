...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    ...
    SibTreeNode nextSibling, prevSibling;
    ...
    if (!isValidNode())
      ...
      if (children() == 0) {
        ...
        nextSibling = (SibTreeNode) child(c);
        if (!nextSibling.isValidNode()){
          if (c < 1) {
            ...
            prevSibling = (SibTreeNode) child(children());
            prevSibling.nextSibling = node;
            node.parent = this;
          }
          myTree.size++;
        } else {
          if (c == 1) { // insert node first
            nextSibling = firstChild;
            node.nextSibling = nextSibling;
            node.parent = this;
            firstChild = node;
          } else {
            prevSibling = (SibTreeNode) child(c-1);
            node.nextSibling = nextSibling;
            prevSibling.nextSibling = node;
            node.parent = this;           
          }
  ...
  public void removeLeaf() throws InvalidNodeException {
    ...
    if (children() == 0){
      ...
      if (parentNode == null) {
        ...
        int index = removeIndex(parentNode, this);
        if (index == 1){
          parentNode.firstChild = this.nextSibling;
        } else {
          SibTreeNode prevSibling = (SibTreeNode) parentNode.child(index-1);
  ...
  public int removeIndex(SibTreeNode parent, SibTreeNode child) {
